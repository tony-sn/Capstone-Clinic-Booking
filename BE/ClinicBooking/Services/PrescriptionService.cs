...
namespace ClinicBooking.Services.IServices
{
    public class PrescriptionService : IPrescriptionService
    {
        ...
        public async Task<PageResultUlt<IEnumerable<PrescriptionDTO>>> GetAll(int pageSize = 0, int pageNumber = 1)
        {
        ...
        public async Task<PageResultUlt<IEnumerable<PrescriptionDTO>>> GetAllByMedicalHistoryId(int id, int pageSize = 0, int pageNumber = 1)
        {
            if (pageSize < 0) throw new ArgumentException($"invalid page size: {pageSize}");
            if (pageNumber <= 0) throw new ArgumentException($"inlvalid page number:{pageNumber}");
        ...
        public Task UpdateTotalAmountAsync(int prescriptionId)
        {
